...
start_rtsp() {
	...
	${cmd} vencstart $1
	${cmd} setbind 7 0 0 $1 $src_fps 2 0 $1 0 $dest_fps
...
start_rtsp_with_dip() {
	...
	${cmd} divpstart $1
	${cmd} setbind 7 0 0 $1 $src_fps 12 0 $1 0 $dest_fps
	${cmd} vencinit $2 $3 $1 $4
	${cmd} setmma 2 0 $1 $mma_name
	${cmd} vencstart $1
	${cmd} setbind 12 0 $1 0 $src_fps 2 0 $1 0 $dest_fps
	${cmd} rtspstart stream$1 $4
	${cmd} q
}
stop_rtsp() {
...
do_baseinit() {
	...
	*)
	...
	if [ "$vpeworkmode" == "0x18" ]; then
		...
		${cmd} setbind2 6 0 0 0 $src_fps 7 0 0 0 $dest_fps 0x4 0
	...
	if [ "$vpeworkmode" == "0x08" ]; then
		...
		${cmd} setbind2 6 0 0 0 $src_fps 7 0 1 0 $dest_fps 0x4 0
		
		${cmd} iqopenserver $vifheight $vifheight 1	
		
		${cmd} vpeinit 0 $vifwidth $vifheight 0x10
		${cmd} setbind 7 0 1 0 $src_fps 7 0 0 0 $dest_fps
