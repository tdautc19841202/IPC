...
static S32 HalPadSetMode_MISC(U32 u32PadID, U32 u32Mode)
{
    ...
    case PAD_FUART_RTS:
        ...
        else if (u32Mode == PINMUX_FOR_PWM0_MODE_6 || u32Mode == PINMUX_FOR_PWM1_MODE_6 || u32Mode == PINMUX_FOR_PWM2_MODE_6 || u32Mode == PINMUX_FOR_PWM3_MODE_6 )
        {
            _HalPadDisablePadMux(PINMUX_FOR_EJ_MODE_1);
            _GPIO_W_WORD_MASK(_RIUA_16BIT(PADTOP_BANK,REG_PWM0_MODE), (BIT1|BIT2)<<((u32PadID-PAD_FUART_RX)*4), (REG_PWM0_MODE_MASK)<<((u32PadID-PAD_FUART_RX)*4));
        }
        else if (u32Mode == PINMUX_FOR_PWM9_MODE_2)
        {   
            _HalPadDisablePadMux(PINMUX_FOR_EJ_MODE_1);
            _GPIO_W_WORD_MASK(_RIUA_16BIT(PADTOP_BANK,REG_PWM9_MODE), BIT5, REG_PWM9_MODE_MASK);
        }
        else if (u32Mode == PINMUX_FOR_PWM0_MODE_6)
        {
            _HalPadDisablePadMux(PINMUX_FOR_EJ_MODE_1);
            _GPIO_W_WORD_MASK(_RIUA_16BIT(PADTOP_BANK,REG_PWM0_MODE), BIT1|BIT2, REG_PWM0_MODE_MASK);
        }
        else
        {
            return -1;
...
S32 HalPadSetVal(U32 u32PadID, U32 u32Mode)
{
    ...
    {
        if(u32PadID == PAD_FUART_RX && u32Mode == PINMUX_FOR_GPIO_MODE)
            return HalPadSetMode_General(u32PadID, u32Mode);
        return HalPadSetMode_MISC(u32PadID, u32Mode);
    }
    else if(u32PadID >= PAD_FUART_RX && u32PadID <= PAD_FUART_RTS)
    {
        if(u32Mode == PINMUX_FOR_GPIO_MODE)
            return HalPadSetMode_General(u32PadID, u32Mode);
        return HalPadSetMode_MISC(u32PadID, u32Mode);
