...
void voltage_level_reconfig (VOLTAGE_DEMANDER_E demander, int voltage)
{
     ...
     if (demander == VOLTAGE_DEMANDER_USER)
     {
     ...
     else
     {
         ...
         else if (voltage == VOLTAGE_CORE_950) {
             _gVoltageDemanderRange[demander].LV = VOLTAGE_CORE_900;
         }
         else {
             _gVoltageDemanderRange[demander].LV = VOLTAGE_CORE_850;
         }
     }
}
...
EXPORT_SYMBOL(sync_core_voltage_with_SIDD_and_TEMP);
#endif
