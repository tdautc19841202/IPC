...
static unsigned long alkaid_poll_wapper(MI_COMMON_PollFileWrapper_t *f, int cmd){
    ...
    res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+E_MI_MODULE_ID_MAX, __pa((long)&ctx), -1, cmd);
    while(1) {
        if (down_timeout(device_sem+ctx.idx, FIVE_SEC) == 0) {
            res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+E_MI_MODULE_ID_MAX, ctx.idx, -1, __MI_DEVICE_QUERY);
            if(res != -2) {
                break;
            } else {
...
static int MI_DEVICE_Open(struct inode *inode, struct file *filp) {
    ...
    res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, __pa((long)&ctx), -1, __MI_DEVICE_CONNECT);
    while(1) {
        if (down_timeout(device_sem+ctx.idx, FIVE_SEC) == 0) {
            res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, ctx.idx, -1, __MI_DEVICE_QUERY);
            if(res != -2) {
                break;
            } else {
                printk(KERN_ERR "bug found at %s %d\n", __func__, __LINE__);
...
static int MI_DEVICE_Release(struct inode *inode, struct file *filp) {
    ...
    res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, __pa((long)&ctx), -1, __MI_DEVICE_DISCONNECT);
    while(1) {
        if (down_timeout(device_sem+ctx.idx, FIVE_SEC) == 0) {
            res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, ctx.idx, -1, __MI_DEVICE_QUERY);
            if(res != -2) {
...
static long MI_DEVICE_Ioctl(struct file *filp, unsigned int cmd, unsigned long ptr) {
    ...
    if(_IOC_TYPE(cmd) == 'i') {
        ...
        if(ptr) {
            ...
            Chip_Flush_MIU_Pipe();
            while(1) {
                res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, __pa((long)&ctx), cmd, __pa((unsigned long)arg));
                if (down_timeout(device_sem+ctx.idx, FIVE_SEC) == 0) {
                    res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, __pa((long)&ctx), cmd, __pa((unsigned long)arg));
                    if(res != -2) {
                        break;
            ...
            while(1) {
                res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, ctxid, cmd, 0);
...
static ssize_t proc_read(struct seq_file* q, void* v)
{
    ...
    res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+pd->modid, __pa((long)&ctx), -1, __MI_DEVICE_PROC_IO);
    ...
    do {
        ...
        invalid_cache_area((void*)logpage, PAGE_SIZE);
        res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+pd->modid, ctx.idx, -1, __MI_DEVICE_PROC_READLOG);
    }while(res);
...
static void fetch_proc(int id, const char *name){
    ...
    res = signal_rtos(INTEROS_SC_L2R_MI_CALL_START+id, __pa(pd), -1, __MI_DEVICE_PROC);
    if(res != __pa(pd)){
        char path[32] = "mi_";
