...
_UVC_Fill_Streaming_Control(ST_UVC_Device_t *uvc,struct uvc_streaming_control *ctrl,int32_t iframe, int32_t iformat);

static int32_t _UVC_FillBuffer(ST_UVC_Device_t *uvc,struct buffer *mem,UVC_IO_MODE_e io_mode)
{
...
static int8_t _UVC_Get_User_Buf_Count(ST_UVC_Device_t *uvc,buffer_status_e status)
{
    if(NULL == uvc->mem)
        return 0;
...
static int8_t _UVC_QBuf(ST_UVC_Device_t *uvc,struct buffer *mem)
{
    int32_t s32Ret;
    ...
    s32Ret = _UVC_FillBuffer(uvc,mem,set.io_mode);
    if( s32Ret <= 0 ) {
...
static struct buffer *_UVC_DQBuf(ST_UVC_Device_t *uvc,bool flags)
{
    int32_t s32Ret;
    ST_UVC_Setting_t set = uvc->ChnAttr.setting;
...
static int8_t _UVC_Video_QBuf(ST_UVC_Device_t *uvc)
{
    uint32_t i;
    ST_UVC_Setting_t set = uvc->ChnAttr.setting;
    struct buffer *mem = NULL;
...
static int8_t _UVC_Video_ReqBufs(ST_UVC_Device_t *uvc,int32_t flags)
{
...
static int8_t _UVC_Video_Stream_on_off(ST_UVC_Device_t *uvc,int32_t fd,int32_t enable)
{
    ...
    if (!enable) {
        if(false==UVC_GET_STATUS(uvc->status,UVC_DEVICE_STREAMON))
           ...
           while(_UVC_Get_User_Buf_Count(uvc,BUFFER_FILLING))
               usleep(1000);

           _UVC_Video_ReqBufs(uvc,0);
        }
        ...
        if(true==UVC_GET_STATUS(uvc->status,UVC_DEVICE_STREAMON))
           goto done;
            ...
            s32Ret = _UVC_Video_ReqBufs(uvc,1);
            if (s32Ret < 0)
                goto done;
...
static void _UVC_SYS_Exit(ST_UVC_Device_t *uvc)
{
    UVC_INFO("\n");
...
static void _UVC_StopCapture(ST_UVC_Device_t *uvc)
{
    UVC_INFO("\n");
...
/* Stream Off the V4l2 Device */
    _UVC_Video_Stream_on_off(uvc,uvc->uvc_fd,0);
}

static int8_t _UVC_SYS_Init(ST_UVC_Device_t *uvc)
{
    if(IS_NULL(uvc))
        return -EINVAL;
...
static int8_t _UVC_StartCapture(ST_UVC_Device_t *uvc)
{
    UVC_INFO("\n");
...
/* Stream On V4l2 Device */
    _UVC_Video_Stream_on_off(uvc,uvc->uvc_fd,1);
                                            ...
                                            struct uvc_request_data *resp)
{
    ...
    case UVC_GET_MAX:
    case UVC_GET_DEF:
        _UVC_Fill_Streaming_Control(uvc,ctrl, req == UVC_GET_MAX ? -1 : 0,
                       req == UVC_GET_MAX ? -1 : 0);
        break;
...
static int8_t _UVC_Events_Process_Setup(ST_UVC_Device_t *uvc,struct usb_ctrlrequest *ctrl,
                                        struct uvc_request_data *resp)
{
...
static int8_t _UVC_Events_Process_Data(ST_UVC_Device_t *uvc,struct uvc_request_data *data)
{
    ...
    if(UVC_STREAMING_INTERFACE == uvc->control.ctype){
        ...
        case UVC_VS_PROBE_CONTROL:
            ...
            uvc->probe = _UVC_Fill_Streaming_Control(uvc,ctrl,0,0);
            break;

        case UVC_VS_COMMIT_CONTROL:
            UVC_INFO(" Commit control, length = %d \n",data->length);
            ctrl = (struct uvc_streaming_control *)&data->data;
            uvc->commit = _UVC_Fill_Streaming_Control(uvc,ctrl,0,0);
            break;
...
static int8_t _UVC_Events_Process(ST_UVC_Device_t *uvc)
{
    ...
    case UVC_EVENT_SETUP:
        _UVC_Events_Process_Setup(uvc,&uvc_event->req, &resp);
        break;

    case UVC_EVENT_DATA:
        s32Ret= _UVC_Events_Process_Data(uvc,&uvc_event->data);
        if (s32Ret< 0)
            break;
        return ST_UVC_SUCCESS;
...
static int8_t _UVC_Video_Process(ST_UVC_Device_t *uvc)
{
    int32_t s32Ret = ST_UVC_SUCCESS;
    struct buffer *mem = NULL;
 ...
 * */
static struct uvc_streaming_control
_UVC_Fill_Streaming_Control(ST_UVC_Device_t *uvc,struct uvc_streaming_control *c,int32_t iframe, int32_t iformat)
{
    int32_t nframes = 0;
...
{
	 ST_UVC_Device_t *uvc = (ST_UVC_Device_t *)arg;
     int32_t s32Ret = -1;
     fd_set fdsu;
     int32_t uvc_fd = uvc->uvc_fd;
...
/* Set default Streaming control */
    ...
    uvc->probe  = _UVC_Fill_Streaming_Control(uvc,NULL,
                    uvc->stream_param.iformat,uvc->stream_param.iframe);
    uvc->commit = _UVC_Fill_Streaming_Control(uvc,NULL,
                    uvc->stream_param.iformat,uvc->stream_param.iframe);
...
int32_t ST_UVC_StartDev(ST_UVC_Device_t *uvc)
{
       ...
       false==UVC_GET_STATUS(uvc->status,UVC_INTDEV_STARTED))
    {
        ...
        pthread_create(&uvc->video_process_thread, NULL, UVC_Video_Process_Task, uvc);
