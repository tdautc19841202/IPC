...
AACLiveFramedSource* AACLiveFramedSource::createNew(UsageEnvironment& env, CONTEXT* ctxt, unsigned char* aacconfig) {
  return new AACLiveFramedSource(env, ctxt, aacconfig);
}

AACLiveFramedSource::AACLiveFramedSource(UsageEnvironment& env, CONTEXT* ctxt, unsigned char* aacconfig)
  ...
  sprintf(fConfigStr, "%02X%02x", aacconfig[0], aacconfig[1]);
}
...
void AACLiveFramedSource::doGetNextFrame() {
    ...
    fDurationInMicroseconds = nread ? fuSecsPerFrame : 0;
