...
int rtsp_servermain(CONTEXT *context, char *pexit) {
  ...
  // A H264/H265 + G711a/AAC video elementary stream:
  {
    ...
    sms->addSubsession(H26XVideoLiveServerMediaSubsession::createNew(*env, context, reuseFirstSource));
    switch (context->audio_enctype) {
    case 0: sms->addSubsession(WAVAudioLiveServerMediaSubsession ::createNew(*env, context, reuseFirstSource)); break;
    case 1: sms->addSubsession(AACAudioLiveServerMediaSubsession ::createNew(*env, context, reuseFirstSource)); break;
    }
