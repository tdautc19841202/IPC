...
OPERATE_RET hwl_wf_station_connect(IN CONST CHAR_T *ssid,IN CONST CHAR_T *passwd)
{
    ...
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "wifi_connect.sh \"%s\" \"%s\"", ssid, passwd);
    system(cmd);
    while(GET_IP_FLAG == 0)
    {
        sleep(1);
...
OPERATE_RET hwl_wf_all_ap_scan(OUT AP_IF_S **ap_ary,OUT UINT_T *num)
{
    ...
    *num = 0;
    ...
    if (pp == NULL) {
        printf("hwl_wf_all_ap_scan popen fails\r\n");
...
OPERATE_RET hwl_wf_get_cur_channel(OUT BYTE_T *chan)
{
    ...
    if (pp == NULL) {
        printf("hwl_wf_get_cur_channel popen fails\r\n");
        return OPRT_COM_ERROR;
    }
...
static pthread_t qrcode_thId ; // ID of qrcode thread
extern int s_enable_qrcode;
...
OPERATE_RET hwl_wf_get_ip(IN CONST WF_IF_E wf, OUT NW_IP_S *ip)
{
    ...
    if(wf == WF_STATION)
    {
        get_dev_ip(WLAN_DEV, ip->ip, 16);
        get_dev_gateway_mask(WLAN_DEV, ip->gw, ip->mask);        
    }
...
OPERATE_RET hwl_wf_get_mac(IN CONST WF_IF_E wf,INOUT NW_MAC_S *mac)
{
    ...
    int    sock;
    struct ifreq ifr = {};
    strncpy(ifr.ifr_name, WLAN_DEV, sizeof(ifr.ifr_name));
    sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (sock < 0) return -1;
    ioctl(sock, SIOCGIFHWADDR, &ifr);
    close(sock);

    mac->mac[0] = ifr.ifr_hwaddr.sa_data[0] & 0xff;
    mac->mac[1] = ifr.ifr_hwaddr.sa_data[1] & 0xff;
    mac->mac[2] = ifr.ifr_hwaddr.sa_data[2] & 0xff;
    mac->mac[3] = ifr.ifr_hwaddr.sa_data[3] & 0xff;
    mac->mac[4] = ifr.ifr_hwaddr.sa_data[4] & 0xff;
    mac->mac[5] = ifr.ifr_hwaddr.sa_data[5] & 0xff;
...
OPERATE_RET hwl_wf_station_disconnect(VOID)
{
    ...
    //Reserved
    system("customer/bin/wifi_disconnect.sh");
    return OPRT_OK;
}
