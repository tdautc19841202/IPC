...
static int wpa_cli_incorrect = 1;
static int wpa_cli_disconnect = 1;
#ifndef CONFIG_CTRL_IFACE_DIR
#define CONFIG_CTRL_IFACE_DIR "/customer/data/cfg/wpa_supplicant"
...
//			wpa_cli_exec(action_file, ifname, "DISCONNECTED");
		...
		wpa_cli_quit = 1;
	} else if (str_starts(pos, WPA_EVENT_PASSWORD_INCORRECT)) {
		if (wpa_cli_incorrect) {
			wpa_cli_exec(action_file, ifname, "INCORRECT");
			wpa_cli_incorrect = 0;
		}
	} else if (str_starts(pos, WPA_EVENT_NETWORK_NOT_FOUND)) {
		if (wpa_cli_disconnect) {
			wpa_cli_exec(action_file, ifname, "NETWORK_NOT_FOUND");
			wpa_cli_disconnect = 0;
		}
	}
}
