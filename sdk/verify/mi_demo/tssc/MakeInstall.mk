...
install:
	@$(STRIP) --strip-unneeded ../out/$(DB_PROJECT)/app/$(MODULE)
	@mkdir -p $(PROJ_ROOT)/release/$(PRODUCT)/$(CHIP)/common/$(TOOLCHAIN)/$(TOOLCHAIN_VERSION)/bin/mi_demo/$(MODULE)/
	@cp -vrf ../out/$(DB_PROJECT)/app/$(MODULE) $(PROJ_ROOT)/release/$(PRODUCT)/$(CHIP)/common/$(TOOLCHAIN)/$(TOOLCHAIN_VERSION)/bin/mi_demo/$(MODULE)/
